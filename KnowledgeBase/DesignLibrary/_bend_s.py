...
from autogds.photonic_utils import get_file_path, model_from_npz
...
def _bend_s(
    ...
    **kwargs,
    ...
    _args = {"size": size, "cross_section": cross_section}
    _args.update(kwargs)
    ...
    # Provide a reasonable length estimate for downstream components (used by gdsfactory couplers).
    try:
        c.info["length"] = float(size[0])
        c.info["min_bend_radius"] = float(max(size) / 2.0)
    except Exception:
        c.info["length"] = 0.0
        c.info["min_bend_radius"] = 0.0
    c.flatten()
    return c
